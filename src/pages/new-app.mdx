...
The _Code Generator_ option will display on the top header in AdminCP Dashboad, next to the User icon.
...
- **App Name**: App name must have to 2 parts in the follow format _vendor_name/app_name_, such as `company/note`
...
Open the file app/settings.json and add the declaration for the Note app ("@metafox/note").
...

{
  ...
  "packages": {
    ...
    "@metafox/report": "local",
    "@metafox/notification": "local",
    "@metafox/static-page": "local",
    "@metafox/poll": "local",
    "@metafox/pages": "local",
    "@metafox/payment": "local",
    "@metafox/photo": "local",
    "@metafox/membership": "local",
    "@metafox/saved": "local",
    "@metafox/search": "local",
    "@metafox/sticker": "local",
    "@metafox/like": "local",
    "@metafox/reaction": "local",
    "@metafox/group": "local",
    "@metafox/friend": "local",
    "@metafox/forum": "local",
    "@metafox/event": "local",
    "@metafox/comment": "local",
    "@metafox/chatplus": "local",
    "@metafox/blog": "local",
    "@metafox/bgstatus": "local",
    "@metafox/announcement": "local",
    "@metafox/feed": "local",
    "@metafox/share": "local",
    "@metafox/emoji": "local",
    "@metafox/activity-point": "local",
    "@metafox/subscription": "local",
    "@metafox/video": "local",
    "@metafox/marketplace": "local",
    "@metafox/quiz": "local",
    "@metafox/chat": "local",
    "@metafox/music": "local",
    "@metafox/advertise": "local",
    "@metafox/livestreaming": "local",
    "@metafox/applepay": "local",
    "@metafox/googlepay": "local",
    "@metafox/emoney": "local",
    "@metafox/invite": "local",
    "@metafox/story": "local",
    "@metafox/site-background": "local",
    "@metafox/featured": "local",
    "@metafox/note": "local"
  }
}
...
We assume that you have set up development environment to build React Native on your machine/server. The _Mobile_ source is located under the `metafox-mobile` folder.

Navigate to folder `metafox` at `metafox-mobile/packages/metafox` and create a new folder for _Notes_ app named `note`.

_Notes_ app should contain the structure as below:
...
Next, we will declare this app name in `apps/settings.json`. 

Now, we will add a new screen for _Notes_ app. On this screen, we will simply show the text "This is note home screen" for now.

Let's create a new file at `metafox-mobile/packages/metafox/note/src/screens/HomeScreen.tsx` and add following code:

```jsx
/**
 * @type: route
 * name: note.home
 * path: /note, /note/:tab(viewAll)
 */
...
function HomeScreen() {
  return (
    <View>
      <Text>This is note home screen</Text>
    </View>
  );
}
...
Now, we will open terminal and run following commands to build the mobile app:
...
After that, you will need to run mobile app using Android Studio or XCode. Once the app starts, you can navigate to note home screen as follows:

```js
const SomeComponent = () => {
  const { navigationBackend } = useGlobal();

  navigationBackend.navigate('note.home');

// OR
// navigationBackend.push('note.home');

// OR
// navigationBackend.openLink('/note');
...
Edit `note/src/screens/HomeScreen.tsx`:
...
Add new layout for this screen in `note/src/layouts.json`:

```js
"note.home": {
    "header": [
      {
        "component": "ui.ModuleHeader", // header component              
        "props": {
          "key": "ui.ModuleHeader",
          "title": "all_notes",
          "rightButtons": [
    ...
    "content": [
      {
        "component": "smarty.SmartResourceList",  // content component
        "props": {
          "key": "smarty.SmartResourceList",
          "hasFilter": true,
          "module_name": "note",
          "resource_name": "note",
          "actionName": "viewAll",  // actionName to get API url
          "skeletonComponent": {  // skeleton component for loading
            "component": "skeleton.note.note"
          },
          "emptyComponent": { // empty component for empty list
            "component": "ui.EmptyList",
            "props": {
              "label": "no_notes_found"
            }
          }
        }
      }
    ...
    "mainBottom": [
      {
        "component": "ui.SortFilterFabButton", // sort/filter button at the bottom
        "props": {
          "key": "ui.SortFilterFabButton"
        }
      }
    ]
  }
...
Then, run the command `yarn config-app` to apply all above changes.
...
A new _Migration_ file is created under `backend/package/company/note/src/Database/Migrations` folder.
...
Then, fill your _App name_ with `company/note`, _schemas_ with `notes`, _Model_ with `Note` and _Entity Name_ with `note`.
...
On the form, choose `company/note` package and `Note` model, then submit.
...
Similar to Frontend, Mobile also supports a built-in dynamic form builder to transform JSON-based responses into React Native Form elements. 

However, most of Mobile's Form elements are different from those in Frontend. So you can find corresponding elements in `metafox-mobile/packages/framework/metafox-form-elements`.

Below is a sample Form response in JSON format:

```txt
{
  "status": "success",
  "data": {
    "component": "Form",     // define React Native render component
    "title": "Add New Note", // form title
    "action": "note",       // target api for http request when form submit.
    "method": "POST",        // http method for http request when form submit.
    "value": {               // initial values.
...
Mobile validation is based on [yup](https://www.npmjs.com/package/yup). _MetaFox_ dynamic form builder transforms JSON object to a `yup` validation object.
...
Similar to Frontend, Mobile supports phrase translation using `i18n` helper:
...
function MyComponent() {
  ...
  return <Text>{i18n.formatMessage({ id: 'value_points' }, { value: 10 })}</Text>;
  // output: <Text>10 points</Text>
}
```
...
See [Route Concepts](/mobile/concepts#route) for detailed guide to set up routes on mobile app.
