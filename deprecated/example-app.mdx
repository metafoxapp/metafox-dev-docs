...
Go to the `company/blog/src/Database/Migrations` directory, open php file

You'll see a class with two method `up` and `down`. We will add code to create schema in `up()` method and destroy schema to `down()` method.
...
To generate example code to a _Resource_, access /admin/apps.
...
If Model has user_id, user_type => use trait **MetaFox\Platform\Traits\Eloquent\Model\HasUserMorph**.

If Model has owner_id, owner_type => use trait **MetaFox\Platform\Traits\Eloquent\Model\HasOwnerMorph**.
...
If Model is Content, and Model can create Feed => implement interface **MetaFox\Platform\Contracts\ActivityFeedSource**.
...
If Model can create Resource Stream (using for browse), implement interface **MetaFox\Platform\Contracts\HasResourceStream**
...
Default Laravel will use class name: MetaFox\Blog\Models\Blog. To use "blog" only, go to company/blog/src/Providers/PackageServiceProvider and define in boot method.
...
Go to company/blog/src/Providers/PackageServiceProvider, define in boot method.
...
Repository can be extended from MetaFox\Platform\Repositories\AbstractRepository, in some case it may not. But a repository always need a repository interface.
...
- MetaFox\Platform\Support\Browse\Scopes\PrivacyScope
...
- MetaFox\Platform\Support\Browse\Scopes\SortScope
...
- MetaFox\Platform\Support\Browse\Scopes\WhenScope
...
As mentioned above, MetaFox\Platform\Support\Browse\Scopes\PrivacyScope can take this logic, just feel free to use it.
...
MetaFox\Platform\Support\Parse\Input
